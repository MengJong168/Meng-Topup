...
PACKAGES_FILE = 'database/packages.json'
...
# Modify the check_payment function to update transaction status
...
def send_to_telegram(transaction):
    ...
    if game_type == 'bloodstrike':
        process_chat_id = '-1002796371372'  # Update with your actual channel ID
        process_text = f"{transaction['player_id']} 0000 {transaction['package_id']}"
    ...
    if game_type == 'mcgg':  # Magic Chess: Go Go
        process_chat_id = '-1002796371372'  # Update with your actual channel ID
        process_text = f"{transaction['player_id']} {transaction['zone_id']} {package_id}"
